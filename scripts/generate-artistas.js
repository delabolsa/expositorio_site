...
// Generar un archivo .astro por cada artista
artistas.forEach(artista => {
  const slug = artista.slug;
  const obrasArtista = obras.filter(o => o.data.artista === slug);

  const content = `---
// Página generada automáticamente para ${artista.data.nombre}
import BaseLayout from "@/layouts/BaseLayout.astro";

const artista = ${JSON.stringify(artista, null, 2)};
const obrasArtista = ${JSON.stringify(obrasArtista, null, 2)};
---
  ...
  <div class="max-w-6xl mx-auto px-4 py-12">
    <div class="grid md:grid-cols-3 gap-10">
      ...
      <div class="md:col-span-2">
        ...
        ${artista.data.biografiaLarga ? `<div class="prose max-w-none mt-6" set:html={${JSON.stringify(artista.data.biografiaLarga)}.replace(/\\n/g, '<br/>')} />` : ''}

        ${artista.data.premios?.length ? `
          <>
            <h2 class="text-2xl font-semibold mt-8">Premios y selecciones</h2>
            <ul class="list-disc list-inside space-y-1">
              ${artista.data.premios.map(p => `<li>${p}</li>`).join('')}
            </ul>
          </>
        ` : ''}

        ${artista.data.participaciones?.length ? `
          <>
            <h2 class="text-2xl font-semibold mt-8">Exposiciones</h2>
            <ul class="list-disc list-inside space-y-1">
              ${artista.data.participaciones.map(p => `<li>${p}</li>`).join('')}
            </ul>
          </>
        ` : ''}

        ${artista.data.experienciaExtra?.length ? `
          <>
            <h2 class="text-2xl font-semibold mt-8">Experiencia extra</h2>
            <ul class="list-disc list-inside space-y-1">
              ${artista.data.experienciaExtra.map(e => `<li>${e}</li>`).join('')}
            </ul>
          </>
        ` : ''}
...
console.log('🎉 Todas las páginas de artistas han sido generadas.');
